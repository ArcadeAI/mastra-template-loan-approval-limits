/**
 * The control plane's boot. Owns `governance.db`, serves Arcade's `/access`,
 * `/pre` and `/post` hooks, and records every decision it makes.
 *
 * Since #4 this is a module of the app, not a service: `bootControlPlane` is
 * called once per process — by `instrumentation.ts` when the app starts, so the
 * cache is warm before the first request, and lazily by `instance.ts` if that
 * has not happened — and the app's routes hand their requests to the handler
 * it returns. `scripts/control-plane.ts` calls the same function and puts the
 * handler on a port of its own, for the harnesses that want the module without
 * booting Next.
 *
 ...
 *
 * Boot order matters: the policy is loaded into memory *before* the first
 * request is served, so the first `/access` Arcade sends — possibly the 1.6 MB one — is
...
import { usingDevSecret, readConfig, type HooksConfig } from "./config.ts";
...
export interface BootOptions {
  ...
  /**
   * Where the handler is being served, for the boot line: `listening on :4401`
   * from the standalone runner (the harnesses read the port off it), `mounted
   * in the app` from the app itself.
   */
  where?: (config: HooksConfig) => string;
}

export function bootControlPlane(options: BootOptions = {}): BootedControlPlane {
  const env = options.env ?? process.env;
  const log = options.log ?? ((line: string) => console.log(`[${SERVICE}] ${line}`));
  ...
  const plane = createControlPlane({ config, db, cache, bus, notices, log, seed, migration });
