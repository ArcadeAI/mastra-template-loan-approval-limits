...
until #8 the TypeScript guard read that copy: it was green while this file
still said "and only then … tell the user who was asked and stop". The same
vocabulary is barred in `tools/loan/tests/test_descriptions.py` and in
`app-test/behaviour.ts`; change all three together.
"""
...
def test_every_description_is_checked():
    # Both tools, so a catalog that silently stopped loading cannot pass by
    # having nothing to check.
    names = {where for where, _ in _sentences() if "." not in where and "->" not in where}
    assert names == {"RequestApproval", "Decide"}
